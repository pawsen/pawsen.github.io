...
Homemade fixture for adding a fixed amount of tension to the lines of a paraglider and providing a target for measuring the line length with a laser. This is pretty neat.

#+hugo: more

A paraglider comes with very specific line length from the manufacture. As the glider get used, the lines might stretch an uneven amount which can change the flight characteristic (specifically trim) and result in poorer performance.
...
#+CAPTION: To get things moving, I made a simple alu-version from a few pieces of scrap and a M5(4.2 mm hole) and M3(2mm hole) tap.
...
2. Know what knot is used to tie the line to the maillon. Different knots uses different lengths of line, which is used for the simulation/adjustment
